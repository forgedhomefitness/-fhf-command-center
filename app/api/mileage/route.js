  ...
  try {
    ...
    const mileageData = {
      ...
      week: {
        ...
        miles: weekMiles,
        deduction: Math.round(weekMiles * IRS_MILEAGE_RATE * 100) / 100,
        dateRange: `${formatDate(weekStart)} - ${formatDate(weekEnd)}`,
      },
      month: {
        trips: monthTrips.length,
        miles: monthMiles,
        deduction: Math.round(monthMiles * IRS_MILEAGE_RATE * 100) / 100,
        dateRange: `${formatDate(monthStart)} - ${formatDate(monthEnd)}`,
      },
      ytd: {
        trips: ytdTrips.length,
        miles: ytdMiles,
        deduction: Math.round(ytdMiles * IRS_MILEAGE_RATE * 100) / 100,
        dateRange: `${formatDate(ytdStart)} - ${formatDate(now)}`,
      },
      trips: body.trips,
    };

    await redisSet("fhf_mileage_data", mileageData);

    return NextResponse.json({
      success: true,
      summary: {
        week: mileageData.week,
        month: mileageData.month,
        ytd: mileageData.ytd,
      },
    });
  } catch (err) {
