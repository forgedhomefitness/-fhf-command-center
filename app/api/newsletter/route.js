...
const NEWSLETTERS = [
  ...
  },
  {
    ...
    studies: [
      { heading: "Study 1: Vitamin D and Calcium Absorption", source: "PMC Expert Consensus (2025)", summary: "Maintain serum vitamin D levels â¥30 ng/mL. Adequate vitamin D increases calcium absorption by 30-40%.", url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC11836767/" },
...
async function getNewsletter(totalWeeks, weekIndex) {
  // Always check Redis first â auto-generated content takes priority
...
export async function GET(request) {
  ...
  try {
    ...
    if (isPreview) {
      // Monday preview â send only to Matt
      ...
      // Tuesday send â to Matt + BCC all clients
      const result = await sendEmail({
        to: MATT_EMAIL,
        bcc: CLIENT_EMAILS,
        subject: newsletter.subject,
