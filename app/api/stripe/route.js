...
export const dynamic = "force-dynamic"; // disable Vercel static caching - must fetch live every call

// Live Stripe fetch - no Redis cache dependency
// Pagination added 2026-06-06: previous version capped at 100 charges,
// which silently dropped March/early-April from yearRevenue.
...
// Fetch ALL charges matching params, following pagination (100/page, max 20 pages)
async function stripeGetAllCharges(params) {
    let all = [];
    let startingAfter = null;
    for (let page = 0; page < 20; page++) {
          const url = `/charges?${params}&limit=100${startingAfter ? `&starting_after=${startingAfter}` : ""}`;
          const batch = await stripeGet(url);
...
export async function GET() {
  ...
  try {
      ...
      // Fetch all data in parallel - week, last week, month, year, customers
      // Year + month use pagination so nothing is dropped past 100 charges.
      const [weekCharges, lastWeekCharges, monthCharges, yearCharges, customers] =
              await Promise.all([
                        ...
                        stripeGetAllCharges(`created[gte]=${monthStart}`),
                        stripeGetAllCharges(`created[gte]=${yearStart}`),
