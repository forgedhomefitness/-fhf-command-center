...
const DEFAULT_CLIENTS = [
  {
    id: 1,
    name: "Alex Tannenbaum",
    type: "backToBack",
    startDate: "2026-03-01",
    notes: "6am sessions, very consistent",
    status: "active",
  },
  {
    id: 2,
  ...
  },
  {
    ...
    notes: "New client, evaluation done",
    status: "active",
  },
];
...
const TYPE_LABELS = {
  ...
  senior30: "Senior 30min",
  senior60: "Senior 60min",
  group: "Group",
};
...
export default function Clients() {
  ...
  useEffect(() => {
    if (clients.length > 0)
      localStorage.setItem("fhf-clients", JSON.stringify(clients));
  }, [clients]);

  useEffect(() => {
    fetch("/api/acuity")
      .then((r) => r.json())
      .then((data) => setAcuity(data))
      .catch(() => {});
  }, []);
  ...
  const totalMonthly =
    acuity?.monthProjectedRevenue ||
    active.reduce((sum, c) => sum + (PRICING[c.type]?.rate || 130) * 4, 0);

  const totalEarned = acuity?.monthEarnedRevenue || 0;
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <p className="text-sm text-dark-400">
            {active.length} active &middot; ${totalEarned.toLocaleString()}{" "}
            earned &middot; ~${totalMonthly.toLocaleString()}/mo projected
            {atRisk > 0 && (
              <span className="text-red-400 ml-2">
                &middot; {atRisk} at risk (14+ days)
              </span>
            )}
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6 mt-4">
        ...
        <div className="card text-center py-3">
          ...
          <p className="text-xs text-dark-400 mt-1">Earned This Month</p>
        </div>
        <div className="card text-center py-3">
      ...
      {atRisk > 0 && (
        <div className="card mb-4 border border-red-500/20 bg-red-500/5">
          <p className="text-xs text-red-400 font-semibold">
            {atRisk} client{atRisk > 1 ? "s" : ""} at risk (14+ days since
            last session)
          </p>
        </div>
      )}
      ...
      {showAdd && (
        <div className="card mb-6 border border-brand-500/20">
          ...
          <textarea
            placeholder="Notes (schedule, goals, preferences...)"
            value={form.notes}
            onChange={(e) => setForm({ ...form, notes: e.target.value })}
            className="w-full bg-dark-800 border border-dark-600 rounded-lg px-3 py-2 text-sm text-white placeholder-dark-500 focus:outline-none mb-3"
            rows={2}
          />
          <div className="flex gap-2">
            <button
              onClick={saveClient}
              className="px-4 py-2 rounded-lg bg-brand-500 text-dark-950 font-semibold text-sm hover:bg-brand-400"
            >
              Save
            </button>
            <button
              ...
              className="px-4 py-2 rounded-lg border border-dark-600 text-dark-300 text-sm hover:text-white"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="space-y-3">
        {clients.map((client) => {
          ...
          const earnedRevenue = ac?.completedRevenue ?? 0;
          const scheduledRevenue = ac?.scheduledRevenue ?? 0;
          ...
          return (
            <div
              key={client.id}
              className={`card flex flex-col sm:flex-row sm:items-center gap-4 ${client.status !== "active" ? "opacity-60" : ""}`}
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {client.status !== "active" && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-dark-700 text-dark-400 uppercase">
                      {client.status}
                    </span>
                  )}
                </div>
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-dark-400">
                  <span className="text-brand-400 font-medium">
                    ${rate}/session &middot; ${earnedRevenue.toLocaleString()}{" "}
                    earned
                    {scheduledRevenue > 0 && (
                      <span className="text-dark-500">
                        {" "}
                        + ${scheduledRevenue.toLocaleString()} scheduled
                      </span>
                    )}
                  </span>
              ...
              <div className="flex items-center gap-4 flex-shrink-0">
                <div className="text-right">
                  ...
                  <p
                    className={`text-sm font-medium ${lastDate ? statusColor(days) : "text-dark-500"}`}
                  >
                    {lastDate
                      ? days === 0
                        ? "Today"
